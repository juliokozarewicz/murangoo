...
from userinfo.models import User_info
...
class LoginView(
    ...
    def form_invalid(self, form):
        for field in form.errors:
            ...
            form.fields[field].widget.attrs.update({'id': 'redborder'})
...
class PasswordResetFromKeyView(
    ...
    def form_valid(self, form):
        ...
        # Remove block user from axes
        userblocked = self.request.META.get('REMOTE_ADDR')
        AccessAttempt.objects.filter(ip_address=userblocked, failures_since_start__gt=0).delete()
...
class account_manager(TemplateView):

    template_name = 'allauth/account/accounts.html'

    # apply filters
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)

        context['profile_page'] = self.get_queryset()['user_profile_page'][0:1]
        return context
    ...
    def get_queryset(self):
        ...
        search = {
            'user_profile_page': User_info.objects.filter(user=self.request.user)
        }
